...
{ 
	...
	{ 
		...
		foreach($arr_data as $key => $value){ 
			if($value['time'] <= $timeout) {
				...
				else $this->data['guestonline']--; unset($arr_data[$key]);
			} 
		}

		//If it's the first hit, add the information to database 
		if(!isset($arr_data[$this->ip])){ 
			$CI =& get_instance(); 

			$arr_data[$this->ip]['time'] = time(); 
			$arr_data[$this->ip]['uri']  = $_SERVER['REQUEST_URI'];

			$username = $CI->session->userdata('username'); 
			$arr_data[$this->ip]['username'] = $username;
			...
			if(!isset($CI->agent)) { 
				$CI->load->library('user_agent'); $class_loaded = true; 
			} 

			if($CI->agent->is_robot()){
				$arr_data[$this->ip]['bot'] = $CI->agent->robot(); 
			}else{
				$arr_data[$this->ip]['bot'] = false;
			}
		...
		$this->data['useronline'] = $arr_data; $this->_save(); 
	}
	...
	function set_data($data=false, $force_update=false)
	{
		...
		foreach($data as $key => $value) {
			if(!isset($arr_data[$this->ip]['data'][$key]) || $force_update) {
				$arr_data[$this->ip]['data'][$key] = $value; 
				$tmp = true; 
			} 
		}
